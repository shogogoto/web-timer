...
- SpaceまたはEnterキーによる開始・一時停止・再開、Escapeキーによる途中終了
- タブやブラウザを閉じても継続するサーバー基準のタイマー
- 終了音、ブラウザ通知、Web Push通知
- 残り時間の割合と一時停止状態を表示するfavicon
- 毎日または曜日・時刻を指定し、任意メッセージも設定できるWeb Pushリマインダー（開始は手動）
- 利用者ごとのタイマー設定保持
- 今日・ページ全体を再読込せず前後へ移動できる週間グラフ・月間カレンダー・時間帯別グラフ
- 一時停止中を除外し、実際に作業した各時間帯へ分けた集計
...
```dotenv
TIMER_IMAGE=ghcr.io/shogogoto/web-timer:0.2.0
```
...
- FirefoxやOSの設定によって、バックグラウンド通知は表示されても通知音が鳴らない場合があります。タブが開いていればアプリ内の終了音とリマインダー音も使用されます。

## リマインダー

タイマー画面上部の「リマインダー」から、集中を始めたい予定を登録できます。

- 「毎日」または曜日を指定
- 時刻は5分刻みで選択し、必要なら分を手動入力
- 5分〜120分のタイマー時間を選択
- 通知本文へ表示するメッセージを任意設定
- 登録後の編集、有効／無効の切り替え、削除

予定時刻になるとWeb Pushで知らせますが、タイマーは自動開始しません。通知を押したとき、タイマーが停止中なら予定時間を選択した画面を開き、開始操作を待ちます。すでに実行中または一時停止中なら、そのタイマーを変更せず現在の画面を開きます。

リマインダー登録時に通知権限とPush購読を確認します。HTTPS環境で利用し、最初に「通知音をテスト」で端末の音量を確認してください。タブが開いていればアプリ内の音を鳴らします。タブを閉じた状態の通知音はブラウザとOSの通知設定に従うため、端末側でもサイト通知の音を有効にする必要があります。
