function studyTimer(initial) {
  return {
    ...
    error: '', interval: null, endAt: null, audioContext: null, pushRegistration: null,
    get display() { const s = this.phase === 'select' ? this.minutes * 60 : this.remaining; return `${Math.floor(s/60)}:${String(s%60).padStart(2,'0')}` },
    init() { window.addEventListener('pagehide',()=>this.pauseOnPageExit()); this.registerPushWorker(); if (this.phase === 'running') this.runClock(); },
